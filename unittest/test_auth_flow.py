...
async def test_complete_auth_flow():
    ...
    try:
        ...
        logger.info(
            f"✓ Received token: {token[:50]}..." if token else "✗ No token received"
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(did_url) as resp:
                if resp.status == 200:
                    ...
                    logger.info("✓ DID document resolution successful")
                else:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(root_url) as resp:
                if resp.status == 200:
                    ...
                    logger.info(
                        f"✓ Root endpoint accessible: {root_response['service']}"
                    )
                else:
...
async def main():
    ...
    logger.info(
        f"Target server: {settings.TARGET_SERVER_HOST}:{settings.TARGET_SERVER_PORT}"
    )
